...
array_push($result, ["level" => "Level " . $i, "total_member" => count($level), "total_income" => round($ttl / 1e18, 5)]);
$i++;
while ($i < 16) {
    $tmp = [];
    $ttl = 0;
    ...
    array_push($result, ["level" => "Level " . $i, "total_member" => count($tmp), "total_income" => round($ttl / 1e18, 5)]);
    $level = $tmp;
    $i++;
}
