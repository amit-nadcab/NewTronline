...
async function withraw_lock(investorId, lock_status ,count) {
  if(lock_status==3){
    return await Withdrawal_lock.updateOne({investorId:investorId}, 
    {count:count});
  }
  else{
    if (lock_status) {
      ...
      return await Withdrawal_lock.insertOne(doc);
    } else {
      const query = {
        investorId: investorId,
      };
...
async function personal_details(req, res) {
  try {
    ...
    communitywithdrawaltrx = communitywithdrawal[0] ? communitywithdrawal[0].sum : 0;
    Object.assign(details, {
      ...
      communitywithdrawal: communitywithdrawaltrx,
...
async function get_community_level_incomes1(req, res) {
  ...
  if (totalId && totalId.length > 0) {
    ...
    while (promotorid >= 0 && i <= 20) {
      ...
      totalId.map((item) => {
        if (item.investorId == promotorid && item.invest_type == "REINVESTMENT") {
    ...
    while (promotorid1 != last_investor + 1 && j <= 20) {
      ...
      totalId.map((item) => {
        if (item.investorId == promotorid1 && item.invest_type == "REINVESTMENT") {
          totalreinvestment += item.trx_amt;
        }
      });
      totalreg.map((item) => {
...
async function withdrawal_request(req, res) {
  try {
    ...
    if (withdraw_locked) {
      withraw_lock(investorId,3,Number(withdraw_locked.count)+1);
      return res.status(400).json({
        status: false,
        message: "Please wait for the confirmation of the previous withdrawal. If this continues contact support!",
      });
    } else {
      withraw_lock(investorId, 1).then(async (data) => {
          console.log("Withraw Lock then ", data)
          if (data) {
            ...
            async function withdrawal(
              ...
              try {
                ...
                if (Number(contract_balance._hex) / 1e6 > Number(total_amt) + 100) {

                  console.log(" Messgafe :", Number(contract_balance._hex) / 1e6, Number(total_amt), waddress,
                    ...
                    if ((checkres && checkres.block) || maxcount > 3) {
                      ...
                      if (
                        ...
                        await Registration.updateOne({
                          ...
                          $set: {
                            wallet_amount: Number(result.wallet_amount) - Number(total_amt),
                          },
                        }).exec((error, data) => {
                          if (error) return res.status(200).json({
                            error
            ...
            if (wallet_balance >= req_withdrawl) {
              if (req_withdrawl >= 100) {
                let req_withdrawl_after_admin_charge = (req_withdrawl * 90) / 100;
                let admin_charge = (req_withdrawl * 10) / 100;
                if (total_investment < 5000) {
                  net_withdrawl = (req_withdrawl_after_admin_charge * 40) / 100;
                  reinvest_amount = (req_withdrawl_after_admin_charge * 60) / 100;
                } else if (total_investment >= 5000 && total_investment < 20000) {
                  if (direct_member >= 5) {
                    net_withdrawl = (req_withdrawl_after_admin_charge * 50) / 100;
                    reinvest_amount = (req_withdrawl_after_admin_charge * 50) / 100;
                  } else {
                    net_withdrawl = (req_withdrawl_after_admin_charge * 40) / 100;
                    reinvest_amount = (req_withdrawl_after_admin_charge * 60) / 100;
                  }
                } else if (total_investment >= 20000 && total_investment < 50000) {
                  if (direct_member >= 8) {
                    net_withdrawl = (req_withdrawl_after_admin_charge * 60) / 100;
                    reinvest_amount = (req_withdrawl_after_admin_charge * 40) / 100;
                  ...
                  if (direct_member >= 10) {
                    net_withdrawl = (req_withdrawl_after_admin_charge * 70) / 100;
                    reinvest_amount = (req_withdrawl_after_admin_charge * 30) / 100;
...
async function vip1_income_withdrawal_request(req, res) {
  try {
    ...
    let withdraw_locked = it.withdraw_locked ? it.withdraw_locked : 0;
    if (withdraw_locked) {
      
      return res.status(400).json({
        status: false,
        message: "Please wait for the confirmation of the previous withdrawal. If it continues, contact support!",
      });
    } else {
      withraw_lock(investorId, 1).then(async (data) => {
          if (data) {
                    ...
                    if ((checkres && checkres.block) || maxcount > 3) {
                      ...
                      if (
                        ...
                        await Registration.updateOne({
                            ...
                            $set: {
                              withdraw_locked: 0,
                              vip_income: Number(it.vip_income) + (Number(req_withdrawl) * 1e6),
                              vip1_wallet: Number(it.vip1_wallet) + (Number(req_withdrawl) * 1e6),
            ...
            if (Number(wallet_balance) / 1e6 >= req_withdrawl) {
              if (req_withdrawl >= 100) {
                console.log("Minus unction", Number(it.vip1_wallet), (Number(req_withdrawl) * 1e6), Number(it.withdraw_vip_income))
                await Registration.updateOne({
                    investorId: investorId
                  }, {
                    $set: {
                      vip_income: Number(it.vip_income) - (Number(req_withdrawl) * 1e6),
                      vip1_wallet: Number(it.vip1_wallet) - (Number(req_withdrawl) * 1e6),
                      withdraw_vip_income: Number(it.withdraw_vip_income) + (Number(req_withdrawl) * 1e6),
                    },
                  }).then(() => {
                    withdrawal(
                      investorId,
                ...
                if (
                  ...
                  await Registration.updateOne({
                    ...
                    $set: {
                      withdraw_locked: 0,
                      vip_income: Number(it.vip_income) + (Number(req_withdrawl) * 1e6),
                      vip2_wallet: Number(it.vip2_wallet) + (Number(req_withdrawl) * 1e6),
                      withdraw_vip_income: Number(it.withdraw_vip_income) -
                        (Number(req_withdrawl) * 1e6),
                    },
                  }).then((error, data) => {
...
async function vip3_income_withdrawal_request(req, res) {
  try {
    ...
    async function withdrawal(
      ...
      try {
        ...
        if (
          ...
          var intervalobj = setInterval(async () => {
            maxcount += 1;
            checkres = await checkwithdraw(result3);
            // console.log("checkres1: ", checkres, Date.now());
            if ((checkres && checkres.block) || maxcount > 3) {
              clearInterval(intervalobj);
              if (
                checkres &&
                checkres.hash == result3 &&
                checkres.contractRet &&
                checkres.contractRet == "SUCCESS"
              ) {
                const withdrawlhistory = new Withdrawlhistory({
                ...
                await Registration.updateOne({
                  ...
                  $set: {
                    vip_income: Number(it.vip_income) + (Number(req_withdrawl) * 1e6),
                    vip3_wallet: Number(it.vip3_wallet) + (Number(req_withdrawl) * 1e6),
                    withdraw_vip_income: Number(it.withdraw_vip_income) -
                      (Number(req_withdrawl) * 1e6),
                  },
                }).exec((error, data) => {
    ...
    //   return res.status(400).json({
    //     status: "false",
    //     message: "Withdrawals are on Hold Due to Server Upgradation and Stabilization!",
    //   });
    // }
  } catch (error) {
    console.log("Error in Withdrawal ", error.message);
...
async function send_all_vip_club_income(req, res) {
  try {
    ...
    // console.log("Count", vip1_count, vip2_count, vip3_count);
    ...
    let single_vip_1_income = vip_1_income / Number(vip1_count);
    ...
    let a = result_vip1.map(async (it) => {
      if (it.vip1_income > single_vip_1_income) {
        const trancsaction = new Transaction({
          investorId: it.investorId,
          random_id: it.random_id,
          wallet_address: it.waddress,
          total_income: single_vip_1_income / 1e6,
          income_type: "VIP 1 INCOME",
          status: 1,
        });
        await trancsaction.save();
        remain_vip_income = remain_vip_income + single_vip_1_income;
        ...
        const trancsaction = new Transaction({
          ...
          random_id: it.random_id,
          wallet_address: it.waddress,
          total_income: it.vip1_income / 1e6,
          income_type: "VIP 1 INCOME",
        });
        await trancsaction.save();
        remain_vip_income += it.vip1_income;
        total_vip_wallet_income =
          Number(total_vip_wallet_income) + Number(it.vip_income);
        await Registration.updateOne({
          _id: it._id
          ...
          $set: {
            ...
            vip1_income: Number(it.vip1_income) - Number(it.vip1_income),
    ...
    Promise.all(a).then(function (res) {
      let b = result_vip2.map(async (it) => {
        if (it.vip2_income > single_vip_2_income) {
          const trancsaction = new Transaction({
            investorId: it.investorId,
            random_id: it.random_id,
            wallet_address: it.waddress,
            total_income: single_vip_2_income / 1e6,
            income_type: "VIP 2 INCOME",
          });
          await trancsaction.save();
          remain_vip_income += single_vip_2_income;
          await Registration.updateOne({
            _id: it._id
          }, {
            $set: {
              vip_income: it.vip_income + single_vip_2_income,
              vip2_income: it.vip2_income - single_vip_2_income,
              vip2_wallet: it.vip2_wallet + single_vip_2_income,
            },
          }).exec();
        } else {
          const trancsaction = new Transaction({
            ...
            random_id: it.random_id,
            wallet_address: it.waddress,
            total_income: it.vip2_income / 1e6,
            income_type: "VIP 2 INCOME",
          });
          await trancsaction.save();
          remain_vip_income += it.vip2_income;
          await Registration.updateOne({
            _id: it._id
          }, {
            $set: {
              vip2: 0,
              vip_income: it.vip_income + it.vip2_income,
              vip2_wallet: it.vip2_wallet + it.vip2_income,
              vip2_income: it.vip2_income - it.vip2_income,
            },
          }).exec();
        }
      });
      Promise.all(b).then(function (res1) {
        let c = result_vip3.map(async (it) => {
          if (it.vip3_income > single_vip_3_income) {
            const trancsaction = new Transaction({
              investorId: it.investorId,
              random_id: it.random_id,
              wallet_address: it.waddress,
              total_income: single_vip_3_income / 1e6,
              income_type: "VIP 3 INCOME",
            });
            await trancsaction.save();
            remain_vip_income += single_vip_3_income;
            await Registration.updateOne({
              _id: it._id
            }, {
              $set: {
                vip_income: it.vip_income + single_vip_3_income,
                vip3_income: it.vip3_income - single_vip_3_income,
                vip3_wallet: it.vip3_wallet + single_vip_3_income,
              },
            }).exec();
          } else {
            const trancsaction = new Transaction({
              ...
              random_id: it.random_id,
              wallet_address: it.waddress,
              total_income: it.vip3_income / 1e6,
              income_type: "VIP 3 INCOME",
            });
            await trancsaction.save();
            ...
            await Registration.updateOne({
              ...
              $set: {
                vip3: 0,
                vip_income: it.vip_income + it.vip3_income,
                vip3_income: it.vip3_income - it.vip3_income,
                vip3_wallet: it.vip3_wallet + it.vip3_income,
              },
            }).exec();
          }
        });
        Promise.all(c).then(function (res2) {
          ...
          Setting.updateOne({
            $set: {
              ...
              total_vip_club: Number(setting_data ? setting_data.total_vip_club ? setting_data.total_vip_club : 0 : 0) + Number(remain_vip_income),
            },
          }).exec();
        });
      });
    });
    console.log(remain_vip_income);
  } catch (error) {
    console.log(" Error! ", error.message);
  }
}
...
async function delete_all_data() {
  ...
  for (i = 0; i < 11; i++) {
    ...
    fetch("http://localhost:3001/api/vip1_income_withdrawal_request/", {
        method: "post",
        headers: {
  ...
  try {
    ...
    async function cf() {
      ...
      if (it.vip1) {
        ...
        if (data) {
          let a = data.map(async (loop) => {
            total_withdraw_vip1 = total_withdraw_vip1 + Number(loop.total_amount);
      ...
      if (it.vip2) {
        ...
        if (data) {
          let a = data.map(async (loop) => {
            total_withdraw_vip2 = total_withdraw_vip2 + Number(loop.total_amount);
            total_withdraw = total_withdraw + Number(loop.total_amount);
          });
          console.log("VIP 2 total_withdraw:: ", total_withdraw_vip2, total_withdraw)
          console.log("Pref 2 total_withdraw:: ", Number(total_withdraw_vip2 * 1e6), tot_vip2_amt)
          Promise.all(a);
          await Registration.updateOne({
            investorId: investorId
          }, {
      ...
      if (it.vip3) {
        ...
        if (data) {
          let a = data.map(async (loop) => {
            total_withdraw_vip3 = total_withdraw_vip3 + Number(loop.total_amount);
            total_withdraw = total_withdraw + Number(loop.total_amount);
          });
          Promise.all(a);
          await Registration.updateOne({
